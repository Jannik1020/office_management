...
export const updateTaskDB = createAsyncThunk("/tasks/update", async ([taskId, currentSection, toUpdate, userId]) => {
    const data = await fetch("http://localhost:8080/api/tasks/update/", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({taskId: taskId, toUpdate: toUpdate, userId: userId})
    }).then(res => res.json()).catch(err => console.error("Error updating task: " + err))

    return [data, currentSection];
})

export const addTaskDB = createAsyncThunk("/tasks/create", async ([task, userId]) => {
...
export const removeTaskDB = createAsyncThunk("/tasks/remove", async ([taskId, userId]) => {
...
export const tasksSlice = createSlice({
    ...
    extraReducers: {
        [fetchTasks.fulfilled]: (state, action) => {
            ...
            tasks.forEach(task => state[task.section].tasks.unshift(createTask({
                title: task.title,
                id: task.id,
                checked: task.checked,
                important: task.important,
                section: Object.entries(defaultSections).filter(section => section[1].name === task.section)[0][1]
            })))
        },
        [updateTaskDB.fulfilled]: (state, action) => {
            ...
            var newTask = createTask({
                ...
                section: Object.entries(defaultSections).filter(section => section[1].name === updatedTask.section)[0][1]
            })
        ...
        [addTaskDB.fulfilled]: (state, action) => {
            ...
            state[task.section].tasks.unshift(createTask({
                title: task.title,
                id: task.id,
                checked: task.checked,
                important: task.important,
        ...
        [removeTaskDB.fulfilled]: (state, action) => {
            ...
            state[task.section].tasks = state[task.section].tasks.filter(iTask => iTask.props.id !== task.id)
        },
